...
function validateEnv() {
  ...
  if (mode === 'LIVE') {
    ...
    if (!process.env.EVM_PRIVATE_KEY && (process.env.ENABLE_BSC === 'true' || process.env.ENABLE_BASE === 'true')) {
      throw new Error('EVM_PRIVATE_KEY required for LIVE mode on BSC/Base');
    }
    if (!process.env.SOLANA_PRIVATE_KEY && process.env.ENABLE_SOLANA === 'true') {
      throw new Error('SOLANA_PRIVATE_KEY required for LIVE mode on Solana');
    }
  }
